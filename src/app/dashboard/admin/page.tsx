...
export default function AdminPage() {
  ...
  const addWorker = async () => {
    ...
    await fetch("/api/users", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: newName.trim(), pin: newPin.trim() }) });
      ...
      <div>
        ...
        <div className="bg-white border border-border rounded-xl p-5 shadow-sm space-y-4">
          ...
          <div>
            ...
            <input value={editSlip} onChange={e => setEditSlip(e.target.value)} placeholder="e.g. 42"
          ...
          <div>
            ...
            <input value={editTask} onChange={e => setEditTask(e.target.value)} placeholder="e.g. Cleaning"
              className="w-full px-3 py-2.5 border border-border rounded-lg text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-100" />
          </div>
          <div>
            <label className="text-xs font-medium text-muted-foreground block mb-1">Notes</label>
            <textarea value={editNotes} onChange={e => setEditNotes(e.target.value)} placeholder="Any notes..." rows={3}
          ...
          <button onClick={saveEdit} disabled={saving} className="w-full py-3 bg-foreground text-white rounded-xl font-semibold text-sm disabled:opacity-50">
        ...
        {audioFiles.length > 0 && (
          <div className="bg-white border border-border rounded-xl p-5 shadow-sm mb-3">
            ...
            <div className="space-y-2">
              {audioFiles.map((p: any) => <audio key={p.id} controls src={p.url} className="w-full" />)}
            </div>
          </div>
        )}
        <div className="bg-white border border-border rounded-xl p-5 shadow-sm mb-3">
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-1.5">
          <button onClick={() => window.location.href = "/api/export"} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium border border-border rounded-lg hover:bg-slate-50 transition-colors">
